...
- The default IQS method advances a coarse implicit time-dependent spatial
  shape and removes the corrector's independent amplitude mode. The accepted
  fine-step amplitude march advances the spatial precursor inventory; a shape
  correction retains and reprojects that inventory rather than replacing it
  with precursors generated by the coarse predictor's different amplitude.
- Shape replacement now retains the adjoint-weighted neutron-population
  coordinate. For fission-normalized old/new shapes it updates the physical
  power factor by
  `q_new/q_old = <phi*,V^-1 psi_old>/<phi*,V^-1 psi_new>`, the integrated
  shape-derivative term. The result exposes the independently marched
  `.amplitude`, the physical `.power`, and `.power_shape_factor`; refreshed
  adjoints are rescaled so their arbitrary eigenvector normalization cannot
  create a population jump.
- `projected_shape_residual` removes the adjoint amplitude component. A soft
  ...
  precursor state.
- The corrector's spatial precursor distribution is retained family by family
  after a positive rescaling makes its adjoint projection exactly match the
  accepted fine-step inventory. `iqs_predictor_tol` adaptively halves future
...
- The three-case 2-D HP-MR CPU matrix in
  `benchmark-results/hpmr-quasistatic-cpu` found and now guards the coarse-
...
where `A` is the adjoint-weighted global neutron-population amplitude and
`psi` is a slowly varying spatial/energy shape. Physical relative fission
power is `P = A q`, where `q` converts the chosen shape normalization to the
adjoint population coordinate. Normalize the shape with a discrete
...
The implementation retains a unit-fission shape for cheap thermal power edits
and applies the algebraically equivalent scalar update
`q_new/q_old = T_old/T_new`, with `T=<psi*,v^-1 psi>`.
...
- Add the shape time-derivative term. **Implemented at shape boundaries through
  the old/new adjoint time-importance ratio.**
