...
The remaining production work is an amplitude-predictor adaptive trigger,
threshold calibration on the one-minute 3-D case, interval-convergence studies,
GPU projection fusion, and multi-interval fallback replay when thermal feedback
changes substantially inside a rejected macro interval.
