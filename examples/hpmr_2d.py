...
outward) to fully inserted (180 deg, arcs facing the core).
    ...
    python examples/hpmr_2d.py [refine] [device]

refine (default 6) sets triangles per hex (6 refine^2) and the drum-arc
rasterization fidelity. Cross sections are placeholders; swap in
SPH-corrected FEMFFUSION sets via ndgpu.femffusion + build_hpmr2d(materials=).
"""
...
print(f"HP-MR 2D, refine={refine} "
      f"({6 * refine**2} triangles/assembly), device={device}\n")
print(f"{'drum angle':>10} {'k_eff':>10} {'reactivity vs 0 (pcm)':>22}")

k0 = None
for angle in (0, 30, 60, 90, 120, 150, 180):
    p = build_hpmr2d(refine=refine, drum_angle_deg=float(angle))
