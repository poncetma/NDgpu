...
from .distributed import (DistributedContext, DistributedResult,
                          DistributedTransientResult, ExtrudedAxialPartition,
                          TriRowPartition)
...
from .distributed_stencil import (DistributedExtrudedMeshGroupOperator,
...
class DistributedTriTransientSolver(TransientSolver):
    """Transient diffusion over MPI-owned triangular row slabs."""
...
class DistributedExtrudedMeshTransientSolver(TransientSolver):
    """Transient diffusion over MPI-owned extruded-mesh axial layers."""
    ...
    def __init__(self, grid, problem_at, kinetics, *, communicator=None,
        ...
        def local_problem_at(time):
            ...
            spec = tuple(problem_at(time))
            if len(spec) not in (2, 4):
                raise ValueError("problem_at must return two or four entries")
            key = tuple(id(value) for value in spec)
            if key == cache_key:
                return cache_value
            if len(spec) == 2:
                ...
                materials, material_map, moving_mix, moving_weight = spec
                cache_value = (
                    materials,
                    _local_array(material_map, partition, "material_map"),
                    _local_array(moving_mix, partition, "mix_material"),
        ...
        def group_operator(xp, ignored_grid, D, removal, **operator_kwargs):
            del ignored_grid
            operator_kwargs.pop("symmetric", None)
            return DistributedExtrudedMeshGroupOperator(
                xp, grid, D, removal, partition, context,
                communication_tag=1700, **operator_kwargs)

        def eigen_solver(ignored_grid, materials, material_map, **solver_kwargs):
            del ignored_grid
            solver_kwargs.pop("device", None)
            return DistributedExtrudedMeshDiffusionEigenSolver(
                grid, materials, material_map, context=context,
                decomposition="axial", **solver_kwargs)
